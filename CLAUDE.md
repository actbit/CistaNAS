...
- **2026-09-09**: パスワード KDF を **Argon2id (RFC 9106) + PBKDF2 合成** に移行（`Shared/Crypto/KdfSpec`・`Argon2Hasher`、Konscious。既定 m=64MiB/t=4/p=4 + 後段 PBKDF2 600k）。ログインハッシュ（Identity、旧 PBKDF2 互換 + 透過再ハッシュ）・ボリューム KEK（ヘッダ `KdfParams` で dispatch）・ブラウザ E2EE（hash-wasm を vendor、`e2ee.js` の `normalizeKdf` で新旧両対応）・Dokan Client / Mobile の全 KEK 導出が対象。RFC 9106 テストベクトル + hash-wasm 相互運用で検証。旧ボリューム・旧 localStorage スキーマは従来 PBKDF2 で自動復号
