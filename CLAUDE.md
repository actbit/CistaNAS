...
- **2026-09-09**: パスワード KDF を **Argon2id (RFC 9106) + PBKDF2 合成** に移行（`Shared/Crypto/KdfSpec`・`Argon2Hasher`、Konscious。既定 m=64MiB/t=4/p=4 + 後段 PBKDF2 600k）。ログインハッシュ（Identity、旧 PBKDF2 互換 + 透過再ハッシュ）・ボリューム KEK（ヘッダ `KdfParams` で dispatch）・ブラウザ E2EE（hash-wasm を vendor、`e2ee.js` の `normalizeKdf` で新旧両対応）・Dokan Client / Mobile の全 KEK 導出が対象。RFC 9106 テストベクトル + hash-wasm 相互運用で検証。旧ボリューム・旧 localStorage スキーマは従来 PBKDF2 で自動復号。**KDF 種別選択**: `argon2id`（合成・既定）/ `argon2id-raw`（Argon2id 単独、PBKDF2 後段なし）を `Volume:KdfAlgorithm` 設定 + Settings UI で選択可（ヘッダ `KdfParams.Algorithm` で永続化、既存ボリュームは従来方式のまま）。raw では `KdfIterations` は不使用（0 で永続化・Validate で範囲チェック対象外）
- **2026-09-09**: ChaCha20-Poly1305 AEAD を **BCL `System.Security.Cryptography.ChaCha20Poly1305`** に移行（サポート環境は CNG/OpenSSL 実装を優先、非対応環境は同梱マネージ実装にフォールバック）。タグ形式を RFC 8439 §2.8 (AAD 対応) に統一 — BCL・マネージ・旧データ（簡略タグ `Poly1305(ct)` のみ）は復号時フォールバック検証で互換維持。**注意**: 旧バージョンのクライアントは新形式 (RFC 8439 タグ) の chacha20 wrapped key / チャンク / ファイル名を復号できない（AES-GCM が既定のため影響は chacha20 選択時のみ）。生 ChaCha20 ストリーム（XTS・旧セクタモード）は BCL に相当 API がないためマネージ実装を維持、ChunkEncryptor の重複実装は Shared 実装に委譲して削除。RFC 8439 §2.8.2 ベクトル + BCL↔マネージ相互運用テストで検証
