...
/// <c>KEK = Argon2id(password, salt, TimeCost, MemoryKiB, Parallelism, len)</c>。
/// Iterations は不使用（永続化では 0）。RFC 9106 準拠の標準構成で、後段変換による
/// エントロピー混合を嫌う場合や相互運用性（他ツールの Argon2id 実装との突合）を優先する場合に選ぶ。
    ...
    int TimeCost = Argon2idKdf.DefaultTimeCost)
{
    ...
    /// <summary>Argon2id 単独（PBKDF2 後段なし。Iterations は不使用）。</summary>
    ...
    public static KdfSpec DefaultArgon2idRaw { get; } =
        new(Argon2idRaw, 0, Argon2idKdf.DefaultMemoryKiB, Argon2idKdf.DefaultParallelism);
    ...
    public bool IsArgon2idRaw => string.Equals(Algorithm, Argon2idRaw, StringComparison.Ordinal);
    public bool IsPbkdf2 => string.Equals(Algorithm, Pbkdf2Sha256, StringComparison.Ordinal);

    /// <summary>Argon2id ファミリ（合成 / 単独いずれか）。MemoryKiB / TimeCost / Parallelism が有効。</summary>
    public bool IsArgon2Family => IsArgon2id || IsArgon2idRaw;

    /// <summary>Argon2id 合成スペックとして検証可能か（検証上限込み）。</summary>
    public bool IsValidArgon2id() =>
        IsArgon2id && Argon2idKdf.IsWithinVerifyLimits(TimeCost, MemoryKiB, Parallelism);

    /// <summary>Argon2id ファミリ（合成 / 単独）スペックとして検証可能か（検証上限込み）。</summary>
    public bool IsValidArgon2Family() =>
        IsArgon2Family && Argon2idKdf.IsWithinVerifyLimits(TimeCost, MemoryKiB, Parallelism);
}
